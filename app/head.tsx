export default function Head() {
  return (
    <>
      <title>Sodynoizz</title>
      <meta name="description" content="Sodynoizz Linktree" />
      <link rel="icon" type="image/jpg" href="/assets/icon.jpg" />
      <meta name="viewport" content="width=device-width, initial-scale=1" />
      <meta name="theme-color" content="#cf1bbe"/>
      <meta name="og:image" content="https://i.imgur.com/Lyxl04B.png" />
      <meta name="og:title" content="Sodynoizz Linktree" />
      <meta name="og:description" content="A sodynoizz linktree website"/>
      <link rel="apple-touch-icon" sizes="180x180" href="/assets/icon.jpg" />
      <link rel="apple-touch-icon" sizes="72x72" href="/assets/icon.jpg"/>
      <link rel="apple-touch-icon" sizes="114x114" href="/assets/icon.jpg"/>
    </>
  );
}
